        ...
        {plans.map((plan, index) => (
          <div key={index} className={`rounded-lg border p-6 shadow-sm ${plan.borderColor}`}>
            <div className="mb-4 flex items-center">
              ...
              <div>
                <h3 className="font-sans text-xl font-extrabold text-gray-900">{plan.name}</h3>
                <p className="text-sm text-gray-600">{plan.description}</p>
              </div>
            </div>
            <p className="font-sans text-lg font-extrabold text-gray-900">{plan.price}</p>
            {plan.details && <p className="mt-1 text-sm text-gray-600">{plan.details}</p>}
            ...
            <ul className="mt-4 space-y-2 text-sm text-gray-600">
              {plan.features.map((feature, featureIndex) => (
                <li key={featureIndex} className="flex items-start">
                  <span className="mr-2 font-sans font-extrabold text-green-600">✓</span>
                  {feature}
                </li>
              ))}
            </ul>
